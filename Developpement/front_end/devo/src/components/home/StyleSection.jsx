...
export default function StyleSection() {
  ...
  const stylesData = [
    {
      ...
      tag: "Shop par style",
      title: "Modern",
      slug: "moderne",                // ← correspond à produits.style en base
    ...
    },
    {
      ...
      tag: "Shop par style",
      title: "Tradition",
      slug: "traditionnel",           // ← correspond à produits.style en base
  ...
  // Navigation vers la page de style
  const handleShopClick = () => {
    navigate(`/styles/${currentStyle.slug}`);
  };
        ...
        >
          ...
          <div className={`flex flex-col justify-center px-12 py-16 transition-all duration-0 ${currentIndex % 2 !== 0 ? 'md:order-2' : 'md:order-1'}`}>
            ...
            <button
              ...
              className="self-start font-manrope border-[1.5px] border-black text-black hover:bg-black hover:text-white text-[12px] font-bold tracking-[0.2em] px-10 py-4 rounded-full transition-all duration-300 uppercase"
            >
              {currentStyle.buttonText}
            </button>
          </div>

          {/* Bloc Image — clic aussi sur l'image */}
          <div
            onClick={handleShopClick}
            className={`relative h-[400px] md:h-auto overflow-hidden cursor-pointer ${currentIndex % 2 !== 0 ? 'md:order-1' : 'md:order-2'}`}
          >
            <img
